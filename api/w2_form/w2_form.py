...
from api.utils.file import allowed_file, format, format_for_llm
...
class W2Form:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                self.w2_form_repo.update_w2_form(
                    w2_form_id, { "gpt_assistant_id": assistant.id, "gpt_thread_id": thread.id }
                )

                return jsonify({"message": 'File successfully processed and data stored', "data": format(w2_forms)}), 200
            else:
    ...
    def saveW2Data(self, data, filename, file_path, user_id):
        ...
        w2_form = self.w2_form_repo.add_w2_form({
            "user_id": user_id,  # TODO: after user auth implementation fetch the user id from request
    ...
    def fetch_w2_data_for_user(self, user_id):
        w2_forms = self.w2_form_repo.fetch_w2_data_for_user(user_id)
        formatted_data = format(w2_forms)

        return jsonify({
            "message": "W2 Forms retrieved successfully",
            "data": formatted_data,
        }), 200
