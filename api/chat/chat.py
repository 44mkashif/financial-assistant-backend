...
class Chat:
    ...
    def question(self):
        ...
        messages = self.assistant.answer_question(
            data.question, 
            data.gpt_assistant_id, 
            data.gpt_thread_id
        )
        answer = messages.data[0].content[0].text.value
        created_at = datetime.fromtimestamp(messages.data[0].created_at).replace(tzinfo=timezone.utc).isoformat()
    ...
    def get_messages(self, gpt_thread_id):
        ...
        sorted_messages = sorted(messages.data, key=lambda x: x.created_at, reverse=False)

        formatted_messages = []
        for message in sorted_messages:
            iso_time = datetime.fromtimestamp(message.created_at).replace(tzinfo=timezone.utc).isoformat()
            formatted_message = {
        ...
        return jsonify({ "message": "Messages retrieved successfully!", "data": formatted_messages })
